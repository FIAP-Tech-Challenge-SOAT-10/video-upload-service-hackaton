...
async def upload_video(file: UploadFile = File(...), user_id: str = Form(...)):
    ...
    if settings.sqs_queue_url:
        try:
            ...
            # não falha o upload; apenas informe que não enfileirou (status segue UPLOADED)
            # em produção, você poderia marcar ERROR ou criar uma DLQ
            pass

    return UploadResponse(id=vid, status=item.status, s3_key=key)
...
def get_download(video_id: str):
    """Opcional nesta v1: já deixo pronto com presigned URL pra fechar o ciclo."""
    item = VideoRepo.get(video_id)
    if not item:
        raise HTTPException(status_code=404, detail="Vídeo não encontrado")
