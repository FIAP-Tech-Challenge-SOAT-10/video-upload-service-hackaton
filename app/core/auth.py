...
def _ensure_client():
    ...
    auth_client = AuthClient(base_url=base_url, timeout_seconds=timeout, cache_ttl=ttl)
...
async def _fetch_me(token: str):
    ...
    try:
        ...
        logger.warning(
            "HTTPStatusError em /me (status=%s url=%s body=%r token_id=%s)",
            sc, url, body, tid
        )
        if sc in (401, 403):
            raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Falha ao validar token no Auth Service"
        )
    except (httpx.TimeoutException, httpx.RequestError) as e:
        logger.error("Erro de rede em /me (token_id=%s): %s", tid, e)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Auth Service inacessível"
        )
